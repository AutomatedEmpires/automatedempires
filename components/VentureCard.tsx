...
export function VentureCard({ venture }: { venture: Venture }) {
  const accentBar: CSSProperties = { backgroundColor: venture.accent };
  const accentText: CSSProperties = { color: venture.accent };
  ...
  return (
    <article className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] p-6 transition hover:border-white/25 hover:bg-white/[0.04]">
      <span className="absolute inset-x-0 top-0 h-0.5" style={accentBar} />
      ...
      <p className="mt-1 text-sm font-medium" style={accentText}>
        {venture.tagline}
      </p>
