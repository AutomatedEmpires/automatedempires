...
| BidSpace | Potential host-funded listing, placement, promotion, or subscription fees for the temporary vendor-space marketplace; live payments/Connect not approved | No BidSpace-specific platform/account was verified | Unknown | Unknown | Keep payment design sandbox-only after legal/entity/operating approval. Do not provision Connect or accept live money until the host-fee and payment/liability model is approved. |
| Lake & Pine | Payments are Phase 2; Phase 1 is booking requests, planning, and cleaning-service operations | No Lake & Pine-specific account was verified | Unknown | Unknown | Do not pull Stripe into Phase 1. Finalize the Phase 2 operating entity, billing/refund workflow, and acceptance gates before test-mode provisioning. |
| Sweepza | Legacy host subscription/additional-listing resources; future monetization must preserve the non-operator discovery model | Dedicated sandbox `acct_1Te…88pB` and live `acct_1Sp…cxzo` are verified. The $19 base and $5 add-on prices resolve, but they are not activation authority. Live retains a foreign E&E webhook, two unclassified customers, and one $0 draft invoice; money activity remains zero | Vercel variables are aligned to the preserved account/mode/prices, but product activation is not approved | **Dedicated technical boundary verified; monetization unapproved; residual objects blocked by production risk** | Keep billing inactive. First approve source/verification/content/disclaimer and legacy-flow disposition, then a non-operator monetization model. Preserve/classify residue without mutation. |
| LogLoads | Subscriptions only; freight brokerage, carrier services, payment processing, dispatch-for-hire, Connect, and freight-payment handling are out of scope | No LogLoads-specific live account was verified | Unknown | Unknown | Preserve the subscriptions-only boundary. Provision only after pricing and operating entity approval; never infer brokerage/payment authority. |
