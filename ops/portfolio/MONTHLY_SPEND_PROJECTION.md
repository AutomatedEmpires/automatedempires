...
|---|---|---:|---:|---:|---|---|---|---|---:|
| PostHog | Existing PAYG | `$0` verified | `$0` verified | `$0` verified | Six projects without upgrade; visible monthly free allowances: 1M analytics events, 5k replay recordings, 1M feature-flag requests, 100k errors, 1,500 survey responses, 1M warehouse rows, 10k pipeline events plus 1M rows, 100k LLM events, 50 GB logs, 10k workflow messages/channel, and 500 PostHog AI credits | Twelve product billing caps set to `$0`; no paid add-ons or upgrades | Parent: AutomatedEmpires. Venture-specific: Explore&Earn, Sweepza, LogLoads, BidSpace, Lake & Pine. ORAN MVP is product-required `no resource` for behavioral analytics/pixels | Events, replay, surveys, flags, errors, and other metered products can create usage charges if caps/settings change. Replay, console capture, and autocapture are disabled on all five new projects | `$0` |
| Mapbox | Pay-as-you-go | `$0` token/base cost verified | Upcoming invoice `$0`; January–July 2026 monthly invoices `$0` | `$0` with four public tokens installed and verified traffic within current evidence | First 50,000 Mapbox GL JS web loads/month are free; token creation has no base cost | No monthly spending cap exists | Venture-specific public tokens: Explore&Earn, LogLoads, BidSpace, Lake & Pine. ORAN/Sweepza/AutomatedEmpires absent | Loads above 50,000/month and other metered APIs can charge. Secret authorizers remain server-side; public `pk` values are client-visible but omitted from docs | `$0` |
...
- Four venture-specific public replacements were installed and exercised through bounded origin/local/Preview verification. This pass added `$0` recurring spend. Usage must still be monitored because there is no spending cap and billable overages/APIs remain possible.
...
4. Track the four installed venture tokens against the 50,000-load allowance and define an alert/manual review because no monthly spending cap exists.
