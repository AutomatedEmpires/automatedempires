...
| One unrestricted default public Mapbox token remains configured in three venture contexts; LogLoads also stores it under a server-shaped variable | Explore&Earn, BidSpace, LogLoads; Lake & Pine has a newly confirmed consumer but no token | Medium | Pass 5 proved all available founder-created named tokens are secret `sk` tokens, unrestricted and at zero usage; they were rejected for browser installation. No value was exposed or changed | **Blocked by security / secure public-token handoff** and downstream production proof | Create one public `pk` token per proven app across its three Doppler configs, verify exact origins/fallbacks through Preview, and retain the shared default until every consumer is replaced and usage is zero |
...
- PostHog now has six separate projects: preserved Explore&Earn plus AutomatedEmpires, Sweepza, LogLoads, BidSpace, and Lake & Pine. Five new projects have autocapture/replay/console capture disabled. Each active venture's own key/host pair is presence-verified in Doppler all lanes and matching Vercel targets; no cross-app reuse was introduced. Fresh deployment/event proof is open, and ORAN remains deferred.
...
Pass 5 added six-project PostHog separation, venture-specific Doppler/Vercel key/host placement, five new-project privacy defaults, and twelve `$0` product caps without an upgrade. It also confirmed four Mapbox source consumers and stopped unsafe installation when every available named replacement proved to be a secret token. It did not claim Mapbox installation/runtime verification, fresh PostHog deployments/events, ORAN analytics, or retirement of the shared public token.
