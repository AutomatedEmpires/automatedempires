...
| Lake & Pine | `lake-and-pine` | Standalone service venture | `AutomatedEmpires/lakeandpine` | Owned `lakeandpinecleaning.com`; authenticated 8-record GoDaddy zone; Vercel web records, no MX/SPF, DMARC quarantine | Dirty `122dd312` artifacts; validated local commit `a55ccff…` unpushed/no Preview; prototype `main` lacks app | Dedicated healthy Supabase in `us-west-1`; backup/lane separation open |
