...
| `RESEND_API_KEY` | Resend API authentication | `dev`, `prd` | `configured` | Same LogLoads key in both lanes; distinct from Explore&Earn but broad/full-access to the same Resend team and failed Explore&Earn domain |
...
- `MAPBOX_ACCESS_TOKEN` is misleadingly named: it duplicates the public token. Remove or rename it unless reviewed server code proves a separate least-privilege token is required. Current authorization lacks `tokens:read` and `tokens:write` for safe inventory/rotation.
...
- `LOGLOADS_EMAIL_FROM` is absent from all configs, so repository code falls back to the provider testing sender. LogLoads has no domain in Resend.
...
- No Cloudinary credential name is present in any LogLoads config; its shared-cloud folder is empty and must remain a placeholder.
