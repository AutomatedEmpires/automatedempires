...
### Public search evidence for brands without portfolio assets

Signed-in Facebook Page searches and direct public Instagram checks were performed on 2026-07-12. They are collision checks, not ownership or trademark determinations.

| Brand | Facebook Page search | Direct Instagram check | Action |
|---|---|---|---|
| AutomatedEmpires | No exact Page result | `@automatedempires` resolves as an empty public profile | Founder must prove control or choose a different handle; do not duplicate |
| Sweepza | Unrelated exact-name robot-cleaning Page | `@sweepza` returned **Profile isn't available** | Do not connect the unrelated Page; recheck Instagram only after founder approval |
| LogLoads | No exact Page result | `@logloads` resolves as unrelated **LoGlo** | Do not use or report the handle; choose a reviewed alternative |
| BidSpace | Several exact-name third-party Pages | `@bidspace` resolves as a private profile | Treat both names as collisions; founder naming decision required |
| Lake & Pine | Several similar-name third-party Pages | `@lakeandpinecleaning` returned **Profile isn't available** | Conduct name-conflict review and recheck before creation |
| ORAN | Numerous unrelated Orange/ORAN results | Full-name candidate unavailable; `@oranetwork` is unrelated **ORA Network e.V.** | Do not use the fallback; recheck the full-name candidate before creation |
...
- No Facebook Page or Instagram profile was visible in the authenticated Meta inventory; the matching asset group exists and is empty.
- If the founder separately approves Page or profile creation, use a conservative category such as **Business service**, **Software**, **Marketing service**, or **Advertising/Marketing**. Never use gambling, casino, lottery, or betting categories or terminology.
- Use the safe pre-launch bio from [`SOCIAL_PROFILE_COPY_BANK.md`](SOCIAL_PROFILE_COPY_BANK.md). The default campaign status is `legal_review_required`; only a separate legal and founder approval may set `approved_for_public`.
- Handle or Page setup does not authorize ads, Pixels, boosted posts, payments, entrant-data collection, scraped or reposted giveaways, or a final compliance claim. Stop if Meta flags the Page, name, category, or copy for review.
...
7. For Sweepza, a founder-approved handle claim is brand preparation only. Keep the account business-facing, leave campaigns at `legal_review_required`, and do not enable ads, Pixels, payments, entrant-data collection, or public campaign activity.
...
- Ownership of the public `@automatedempires` Instagram profile and whether it is founder-controlled.
- Whether any unobserved social accounts for these brands exist outside the authenticated switcher and exact searches recorded above.
