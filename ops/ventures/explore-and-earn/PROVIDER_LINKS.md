...
| --- | --- | --- | --- |
| GitHub | [AutomatedEmpires/explore-and-earn](https://github.com/AutomatedEmpires/explore-and-earn) | **Current source includes #244/#245 through `5989853`; protection/security fixed** | Preserve strict `verify`/`design`/`migration`, squash/linear history, conversation resolution, and read-only default workflow permissions. |
...
| Vercel | [Vercel dashboard](https://vercel.com/dashboard) — project `explore-and-earn`; production `dpl_5HCPaCNCQuyip2iZTHpoMvSxjQFY` | **Prior-source production `READY`; current source later** | Git/main/root and custom domain are verified for deployed source `b616b9e10fa434422dd34442f6cb24194cf8d5ec`; current source #245 `5989853` needs deployment revalidation. |
...
| PostHog | [PostHog](https://app.posthog.com/) — project `291166` | **Existing project preserved; environment binding verified; post-write runtime pending deployment** | Own key/host spans Doppler/Vercel targets; source privacy defaults retained. |
...
| Mapbox | [Mapbox account](https://console.mapbox.com/) | **Venture token installed; origin/local proof green; Preview app path open** | Seven restrictions; all Doppler/Vercel lanes populated; shared provider token stays rollback. |
