...
| --- | --- | --- | --- |
| Vercel settings and protected PR preview are verified; an earlier production-target artifact is `ERROR`, and no functional production or custom domain exists. | **Verified current** | Merge still needs a clean main-based production proof before DNS cutover. | Review/merge, verify a functional production provider hostname and rollback, then plan domain cutover; do not promote the errored artifact. |
