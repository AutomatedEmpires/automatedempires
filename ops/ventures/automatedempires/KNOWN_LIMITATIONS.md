...
| --- | --- | --- | --- |
| Current source includes merged #12 at `408fca3`; clean production `dpl_4b8tDpSmz71WTGmrgRigPb7k5bT9` is last-deployed evidence from `664cda67bc7bc91e4f1577d76bbe7758290f3a2d`, and functional rollback/DNS smoke remains. | **Current/deployed source split / production-risk gate** | Current source must be revalidated against the deployed artifact; functional rollback proof still precedes DNS. | Verify current-source deployment/provider-host behavior and rollback, then plan domain cutover. |
...
| The current public surface has no Supabase requirement. Future command-layer data architecture is undecided. | **Verified current surface / open command-layer design** | A premature database would add cost/access risk, while a later evidence command layer may need a reviewed store. | Preserve no database for the current surface; any future design must cover access, privacy, cost, ownership, evidence freshness, and removal. |
