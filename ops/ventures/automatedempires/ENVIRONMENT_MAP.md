...
| PostHog | **Provider/environment boundary fixed; runtime pending deployment** | Project `509081`; privacy defaults off. Doppler `dev` pair is restricted/browser write-confirmed and CLI-null by design; `stg`/`prd` presence verified. Vercel Development directly repaired/exact-match verified; combined Preview/Production present. No deploy/event smoke. |
