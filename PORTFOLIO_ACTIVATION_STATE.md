...
| BidSpace | AutomatedEmpires/bidspace | not selected | none verified | SOFTWARE COMPLETE · ENGINE VERIFIED · SENTRY LIVE · FOUNDER-GATED (Clerk + Stripe) |
| ORAN | AutomatedEmpires/Open-Resource-Access-Network | not selected | none verified | AZURE-FREE + VERCEL-NATIVE · Supabase live `tpatxospkuqvajusuryw` (RLS locked) · Clerk code-complete · build green · LAUNCH founder-gated (Vercel deploy + Clerk app + DATABASE_URL + data — no automation API) |
...
- Stripe Connect: **architecture MODERNIZED (D028)** — controller-property accounts (off deprecated `type:express`); destination-charge/fee/webhook code current and unchanged; webhook settlement hardened (process-first, record-on-success) so a crash mid-settlement can’t strand a paid booking. Activation gated on founder KYC; runbooks: `docs/CONNECT-RUNBOOK.md`, `docs/MONEY-PROOF-FIXTURE.md`. **Open question (2026-07-10): which Stripe account?** The claude.ai Stripe connector is bound to `acct_1RMjIWIH4Hw2pSG9` ("explore&earn", zero connected accounts on it) — a *different* id than the shared KYC-complete `acct_1SpxXpDtcwz0cxzo` that Sweepza/LogLoads reuse. Not resolved unilaterally; needs a founder call.
...
- **Sentry: DONE.** Project `bidspace` provisioned in org `automated-empires` via the claude.ai Sentry connector; `@sentry/nextjs` fully wired (instrumentation.ts/instrumentation-client.ts, error boundaries, `sendDefaultPii:false`); event delivery **proven live** — `tools/verify-sentry.ts` landed as issue `BIDSPACE-1`, confirmed via the Sentry API, resolved as a test. No founder action needed.
- Mapbox / PostHog: code complete and env-gated; `/map` degrades honestly; 7 PII-free analytics events instrumented. Both connectors available to the agent cannot provision new accounts (Mapbox: no connector; PostHog: query-only, scoped to exploreandearn) — still a founder key/project each. Env contract: `docs/ENVIRONMENT.md`.
- Founder gates: **(1)** create the Clerk application + provide production keys; **(2)** decide the Stripe account + complete KYC/beneficial-owner/bank; **(3)** PostHog project + key (small); **(4)** Mapbox token (small).
- State: **SOFTWARE COMPLETE · INTERNAL ENGINE VERIFIED · CONNECT MODERNIZED · SENTRY LIVE · EXTERNAL IDENTITY & MONEY FOUNDER-GATED**.

### ORAN — Open Resource Access Network

Convergence pass 2026-07-08 (Claude Opus 4.8, repo-resident). Civic human-services resource-matching platform.

- Repository: `AutomatedEmpires/Open-Resource-Access-Network`. Was **not** checked out in the `ventures/` workspace (only an ephemeral `/tmp` shallow clone) — cloned to `~/automatedempires/ventures/oran` this pass. Single Next.js 16 app; strong preserved domain logic (deterministic 3-score ranking `0.45·verification+0.40·eligibility+0.15·constraint`, crisis hard-gate, retrieval-first no-fabrication chat, HSDS/211 federation, PostGIS + pgvector, import→verify→publish ingestion).
- **Azure exit (application layer): DONE + VERIFIED GREEN** on branch `feat/portfolio-convergence-azure-exit` (**pushed**; HEAD `6add79a`). Migrated adapters to the portfolio stack: telemetry AppInsights→**Sentry**, email ACS→**Resend**, maps Azure Maps→**tokenless OpenStreetMap/Leaflet** (removed SAS token broker + rotate-sas workflow), geocoding→**OSM Nominatim**, seeker LLM Azure OpenAI→**OpenAI** (Azure kept as fallback). Removed deps `@azure/communication-email`, `applicationinsights`, `azure-maps-control`; reconciled the runtime env contract + `.env.example`. Verification: `tsc` pass, `eslint` pass, `next build` pass, unit suite 3836 pass / 3 pre-existing wall-clock-dependent failures (unchanged).
- **Safety:** fixed a crisis hard-gate first-person bypass (a first-person disclosure wrapped in "what should I do if…" / "how do I help…" was demoted below the hard-stop) with regression tests; the classifier can now only ever promote to crisis, never demote.
- **Database → Supabase: SCHEMA LIVE.** Created project **`oran` = `tpatxospkuqvajusuryw`** (us-east-1, Postgres 17); applied all 53 migrations via the Management API (100 tables, postgis+pgvector+uuid-ossp, seeds; verified live). App wired (`src/db/ssl.ts` auto-TLS, removed unused `@neondatabase/serverless`). Founder gate: set `DATABASE_URL` (DB password is dashboard-only). ⚠️ RLS is disabled on all tables — app is pg-direct (not Supabase-client); recommend REVOKE from anon/authenticated or deny-all RLS (docs/CONVERGENCE.md; not auto-applied).
- **Auth → Clerk: CODE-COMPLETE.** Identity=Clerk; authorization stays DB-driven (`user_profiles.role` + `organization_members`). One server seam (`getAuthContext` on Clerk `auth()`, on-demand seeker-profile creation) migrated all ~100 API routes; a NextAuth-shaped client compat hook migrated ~13 files by import; `ClerkProvider` + `clerkMiddleware` (works as Next 16 `proxy.ts`) + `<SignIn/>`/`<SignUp/>`. Removed `next-auth`. Founder gate: create the Clerk app (dashboard-only, no API), set `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` + `CLERK_SECRET_KEY`; live sign-in flow untested without it.
- **Infra → Vercel-native: DONE.** Removed the `functions/` Azure Functions runtime, `infra/` Bicep, the 3 deploy-azure workflows, and `build:functions`. Added `vercel.json` + Vercel cron routes (`src/app/api/cron/*` via `src/lib/cron.ts`, CRON_SECRET-gated) replacing the 5 timer functions. **No `@azure/*` dep, no Bicep, no Azure Functions, no Azure workflows remain.** `next build` compiles the cron routes. Supabase RLS locked down (anon/authenticated revoked + deny-all RLS; app's pg-direct owner bypasses).
- **Launch is founder-gated — NO automation API for any of these** (verified this pass): (1) **Vercel deploy** — no Vercel token exists (CLI unauthenticated, none in Doppler/env; no ORAN Vercel project); import the repo into the AutomatedEmpires Vercel team. (2) **Clerk app** (dashboard-only). (3) **`DATABASE_URL`** (Supabase dashboard). (4) real resource **data** (ingestion/import). Exact steps in repo `docs/CONVERGENCE.md` §8. Public seeker surfaces work with just DATABASE_URL + data; Clerk gates host/admin/saved (dormant).
- **Code follow-ups:** crawl-pipeline queue substitute (not launch-critical), ingestion LLM provider→OpenAI, Translator/Speech provider-neutral, controlPlane self-model text, pnpm.
- State: **AZURE-FREE · VERCEL-NATIVE · SUPABASE LIVE (RLS locked) · CLERK CODE-COMPLETE · BUILD GREEN — launch blocked only on 3 founder dashboard actions (Vercel/Clerk/DATABASE_URL) + data, none of which have an accessible API.**
